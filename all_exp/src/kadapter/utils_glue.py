...
class trex_rcInputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, subj_special_start_id, subj_special_end_id, obj_special_start_id, obj_special_end_id):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = label_id
        self.subj_special_start_id = subj_special_start_id
        self.subj_special_end_id = subj_special_end_id
        self.obj_special_start_id = obj_special_start_id
        self.obj_special_end_id = obj_special_end_id
...
def convert_examples_to_features_trex(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if subj_special_end >= max_seq_length:
            continue
            # subj_special_end = max_seq_length - 10
        if obj_special_start >= max_seq_length:
            continue
            # obj_special_start = max_seq_length - 10
        if obj_special_end >= max_seq_length:
            continue
            # obj_special_start = max_seq_length - 10

        subj_special_start_id = np.zeros(max_seq_length)
        subj_special_end_id = np.zeros(max_seq_length)
        obj_special_start_id = np.zeros(max_seq_length)
        obj_special_end_id = np.zeros(max_seq_length)
        subj_special_start_id[subj_special_start] = 1
        subj_special_end_id[subj_special_end] = 1
        obj_special_start_id[obj_special_start] = 1
        obj_special_end_id[obj_special_end] = 1
        ...
        features.append(
            trex_rcInputFeatures(input_ids=input_ids,
                                ...
                                subj_special_end_id=subj_special_end_id,
                                obj_special_start_id=obj_special_start_id,
                                obj_special_end_id=obj_special_end_id))
    return features
